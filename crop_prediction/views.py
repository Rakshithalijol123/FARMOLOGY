...
#     res = convert_to_num(commodity_name,state,district,market,month_column,season_names)
#     res.append(day)
#     res = [res,]
#     print(f"############{res}#########################")
#     return HttpResponse(answer(res))
...
def index(request):
    if request.method == "POST":
        ...
        month_column = request.POST.get('month_column')
        season_names = request.POST.get('season_names')
        day = request.POST.get('day')
        # print(f"#################{commodity_name},{state}#######################")
        model = joblib.load("final.joblib")
        return HttpResponse(model.predict([[11,11,11,11,1,1,1]]))
